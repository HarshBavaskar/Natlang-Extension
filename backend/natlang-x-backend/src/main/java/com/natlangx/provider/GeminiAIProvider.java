...
public class GeminiAIProvider extends AIProvider {
    ...
    @Value("${natlangx.gemini.apiKey:}")
    private String apiKey;

    @Value("${natlangx.gemini.baseUrl:https://generativelanguage.googleapis.com}")
    private String baseUrl;

    @Value("${natlangx.gemini.model:gemini-1.5-flash}")
    ...
    public ProviderHealthStatus health() {
        ...
        if (!configured) {
            return new ProviderHealthStatus(providerName(), false, false, "Missing NATLANGX_GEMINI_API_KEY.");
        }

        String endpoint = stripTrailingSlash(baseUrl) + "/v1beta/models?key=" + apiKey;
        try {
            ...
            return new ProviderHealthStatus(providerName(), true, ok, ok
                    ...
                    : "Gemini returned status " + response.statusCode());
        } catch (InterruptedException ex) {
            Thread.currentThread().interrupt();
            return new ProviderHealthStatus(providerName(), true, false, "Gemini health check interrupted: " + ex.getMessage());
        } catch (IOException | RuntimeException ex) {
            return new ProviderHealthStatus(providerName(), true, false, "Gemini unreachable: " + ex.getMessage());
        }
    }
    ...
    private String generate(String instruction) {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException("Gemini API key is not configured. Set NATLANGX_GEMINI_API_KEY.");
        }

        String endpoint = stripTrailingSlash(baseUrl) + "/v1beta/models/" + model + ":generateContent?key=" + apiKey;
        String payload = "{"
                + "\"contents\":[{" 
                + "\"parts\":[{" 
                ...
                + "}]"
                + "}],"
                + "\"generationConfig\":{\"temperature\":0,\"topP\":0}"
                + "}";
        ...
        try {
            ...
            if (content.isBlank()) {
                throw new RuntimeException("Gemini response did not include generated text.");
            }
            return content;
        } catch (InterruptedException ex) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Failed to call Gemini provider: " + ex.getMessage(), ex);
